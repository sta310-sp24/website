...
Then, fill in the code to calculate summary statistics for the variable using the `skim` function from the skimr R package [@skimr].
...
::: callout-tip
See [Univariate EDA](https://introregression.netlify.app/04-slr#univariate-eda) for an example.
:::
...
-   Fit and display a linear regression model of the relationship between `mean_spend` and `mean_pts_near`. Use the tidymodels [@tidymodels] syntax and display the model in a tidy format.
