...
-   Go to the course organization at [github.com/sta210-fa23](https://github.com/sta210-fa23) organization on GitHub. Click on the repo with the prefix **lab-01-**. It contains the starter documents you need to complete the lab.
    -   If you do not see your lab-01 repo, [click here](https://classroom.github.com/a/5kU1kpeo) to create your repo. Then, [click here](https://forms.office.com/r/S1eSDTjB6M) to (re)submit your GitHub username.
...
-   `pct_near_park_points`: Points in the Trust for Public Land ParkScore index for the percent of residents within a 10 minute walk to park points. [Click here](https://www.tpl.org/parkscore) to learn more about the ParkScore.
...
The predictor variable for this analysis `spend_per_resident_data` is quantitative; however, from the glimpse of the data in Exercise 1, we see its data type is `chr` (character) in R even though we would expect it to be `dbl` (double), the data type for numeric data.

-   Why did `spend_per_resident_data` get read by R as a character data type instead of a double?
-   Why do we need to convert `spend_per_resident_data` to a double data type? How might leaving it as a character variable hinder the analysis?

### Exercise 3

Below is code below to convert `spend_per_resident_data` , so that it is correctly treated as quantitative data in R. Each line of code is numbered. Write a brief explanation of what each line of code does.
...
Exploratory data analysis (EDA) helps us "get to know" the data and examine the variable distributions and relationships between variables. We do using visualizations and summary statistics to describe the distirbutions of the variables in our data set.
...
Now fill in the code to calculate summary statistics for the variable using the `skim` function from the skimr R package [@skimr].
...
<!-- -->
```
-   Use the visualization and correlation to describe the relationship between the two variables. Include the shape, direction, strength, presence of potential outliers, and other any other interesting features in the description.
...
-   Fit and display a linear regression model of the relationship between spending and points for residents within a 10-minute walk from a park. Use the tidymodels [@tidymodels] syntax to fit the model. Display the model in a tidy way.

-   Use the model to describe the impact of spending on park access in the 100 most populated cities. Be specific. Use interpretations of the relevant model coefficient(s) in your response.
