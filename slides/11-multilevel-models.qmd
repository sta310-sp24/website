...
::: appex
📋 [sta310-sp24.netlify.app/ae/lec-11-multilevel](https://sta310-sp24.netlify.app/ae/lec-11-multilevel)
:::
...
`r emo::ji("two")` Fit a separate model for each musician understand the association between performance type (Level One models) and anxiety. Then fit a system of Level Two models to predict the fitted coefficients in the Level One model for each subject based on instrument type (Level Two model).
...
$$\begin{aligned}&\hat{a}_i = 15.706 + 2.205 ~ Orchestra_i \\
&\hat{b}_i = -0.771 - 1.406 ~ Orchestra_i\end{aligned}$$

## Estimated composite model {.midi}

$$
\begin{aligned}\hat{na}_{ij} &= 15.706 + 2.205 ~ Orchestra_i - 0.771 ~ LargeEnsemble_{ij} \\
...
`r emo::ji("warning")` Does not share strength effectively across individuals (look at $R^2$ values in Part 3: Level One Models of AE)
...
-   For the Level Two models, the errors are
    -   $u_i$: deviation of musician $i$ from the mean performance anxiety before solos and small ensembles after accounting for the instrument
        -   musician-to-musician differences in intercept
