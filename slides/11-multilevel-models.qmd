...
::: appex
📋 [sta310-sp24.netlify.app/ae/lec-11-mutlilevel](https://sta310-sp24.netlify.app/ae/lec-11-mutlilevel.html)
:::
...
We'll start with the Level One model to understand the association between performance type and performance anxiety for the $i^{th}$ musician and the $j^{th}$ performance $$na_{ij} = a_i + b_i ~ LargeEnsemble_{ij} + \epsilon_i, \hspace{5mm} \epsilon_{ij} \sim N(0,\sigma^2)$$
...
$$\begin{aligned}&\hat{a}_i = 15.721 + 1.441 ~ Orchestra_i \\
&\hat{b}_i = -0.771 - 1.406 ~ Orchestra_i\end{aligned}$$

## Estimated composite model {.midi}

$$
\begin{aligned}\hat{na}_{ij} &= 15.721 + 1.441 ~ Orchestra_i - 0.771 ~ LargeEnsemble_{ij} \\
