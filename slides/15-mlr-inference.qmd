...
As with SLR, we use $\hat{\sigma}_{\epsilon}$ to calculate $SE_{\hat{\beta}_j}$, the standard error of each coefficient.
...
-   Include a quadratic term in the model mean-centering the variable first
...
As before, the constant variance condition is not satisfied. We can transform the response variable to address the violation in this condition.
...
$$\widehat{\text{Median}({Y|X_1,\ldots,X_p})} = \exp\{\hat{\beta}_0\}\exp\{\hat{\beta}_1X\}\dots\exp\{\hat{\beta}_pX_p\}$$

::: callout-note
If the distribution of $Y|X_1, \ldots, X_p$ is symmetric, then the median equals the mean.
:::
...
-   **Intercept**: When $X=0$, $Y$ is expected to be $\exp\{\hat{\beta}_0\}$

-   **Slope:** For every one unit increase in $X$, the $Y$ is expected to multiply by a factor of $\exp\{\hat{\beta}_1\}$
