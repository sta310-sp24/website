...
$$\begin{aligned}\log\Big(\frac{p_{i[gh]j}}{1 - p_{i[gh]j}}\Big) &= \alpha_0 + \beta_0 ~ \text{foul.diff}_{ij} + \gamma_0 ~\text{score.diff}_{ij} + \delta_0 ~ \text{lead.home}_{ij} \\ 
...
& + \omega_0 ~ \text{foul.diff}_{ij}\text{:time_}_{ij}+ \zeta_0 ~ \text{lead.home}_{ij}\text{:time}_{ij} \\ 
&+ [u_i + v_h + w_g]\end{aligned}$$
$$u_i \sim N(0, \sigma^2_u) \hspace{10mm} v_h \sim N(0, \sigma^2_v) \hspace{10mm} w_g \sim N(0, \sigma^2_w)$$
...
```{r}
#| eval: false
model_full <- glmer(foul.home ~ foul.diff + score.diff + lead.home + time + 
                      offensive + personal + foul.diff:offensive + 
                      foul.diff:personal + foul.diff:time + lead.home:time + 
                      (1|game) + (1|hometeam) + (1|visitor),
...
model_full <- glmer(foul.home ~ foul.diff + score.diff + 
...
<!--# Maybe see the conclusions from the paper and see if it's consistent?-->
